...
$active_guaranties = 0;
try {
    $total_guaranties = $pdo->query("SELECT COUNT(*) as total FROM guaranty_cards")->fetch()['total'];
    
    // محاسبه گارانتی‌های فعال (فرض می‌کنیم گارانتی 18 ماه اعتبار دارد)
    $stmt = $pdo->prepare("SELECT COUNT(*) as total FROM guaranty_cards WHERE DATE_ADD(issue_date, INTERVAL 18 MONTH) >= CURDATE()");
    $stmt->execute();
    $active_guaranties = $stmt->fetch()['total'];
} catch (PDOException $e) {
    // اگر جدول guaranty_cards وجود ندارد، خطا را نادیده می‌گیریم
}
